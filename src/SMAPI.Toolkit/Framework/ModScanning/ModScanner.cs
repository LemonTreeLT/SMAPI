        ...
        public ModFolder ReadFolder(DirectoryInfo root, DirectoryInfo searchFolder, bool useCaseInsensitiveFilePaths)
        {
            ...
            if (manifestFile == null)
            {
                ...
                if (this.IsXnbMod(relevantFiles))
                    return new ModFolder(root, searchFolder, ModType.Xnb, null, ModParseError.XnbMod, I18nUtilities.Get("console.mod-scanner.not-a-mod", null));

                // SMAPI installer
                if (relevantFiles.Any(p => p.Name is "install on Linux.sh" or "install on macOS.command" or "install on Windows.bat"))
                    return new ModFolder(root, searchFolder, ModType.Invalid, null, ModParseError.ManifestMissing, I18nUtilities.Get("console.mod-scanner.smapi-installer-isnt-mod", null));
            ...
            string? errorText = null;
            {
                ...
                {
                    error = ModParseError.ManifestInvalid;
                    errorText = I18nUtilities.Get("console.mod-scanner.error-parsing-manifest-failed", new {msg = ex});
                }
            }
